...
class OptionsEngine:
    ...
    def simulate_contract_price(self, spot_history, strike_pct=1.0, dte=7, initial_vol=0.20, steps_per_day=26):
        """
        Simulates how an option contract price changes over a price history.
        steps_per_day: Number of intervals in a trading day (e.g. 26 for 15m, 78 for 5m)
        """
        ...
        for i, S in enumerate(spot_history):
            # Remaining time (DTE decreases per step)
            current_dte = dte - (i / float(steps_per_day)) 
